...
exports.runBacktestWithParams = runBacktestWithParams;
exports.runOptimization = runOptimization;
exports.printOptimizationResults = printOptimizationResults;
const indicators_1 = require("./indicators");
/**
 * Run backtest with custom parameters (for optimization)
 */
async function runBacktestWithParams(candles, params) {
 ...
 */
async function runOptimization(symbol, timeframe, daysBack) {
    ...
    for (const rsi of rsiPeriods) {
        for (const sl of stopLossPcts) {
            for (const tp of takeProfitPcts) {
                current++;
                const params = { rsiPeriod: rsi, stopLossPct: sl, takeProfitPct: tp };
 ...
 */
function printOptimizationResults(results, topN = 5) {
    console.log('\n' + '='.repeat(80));
